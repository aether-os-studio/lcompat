...
struct ieee80211_txq {
    ...
    struct ieee80211_vif *vif;
    u8 tid;
    struct sk_buff_head lcompat_queue;
    char drv_priv[];
};
...
ieee80211_probereq_get(struct ieee80211_hw *hw, const u8 *src_addr,
    ...
    if (ssid_len > U8_MAX)
        return NULL;
    if (lcompat_ieee80211_size_add(sizeof(struct ieee80211_hdr_3addr),
                                   2 + ssid_len, &frame_len))
        return NULL;
...
ieee80211_proberesp_get(struct ieee80211_hw *hw, struct ieee80211_vif *vif) {
    ...
    skb = lcompat_ieee80211_alloc_helper_skb(
        hw, sizeof(struct ieee80211_hdr_3addr), 0);
    if (!skb)
        return NULL;
    hdr = skb_put_zero(skb, sizeof(struct ieee80211_hdr_3addr));
    if (!hdr) {
...
lcompat_ieee80211_nullfunc_get(struct ieee80211_hw *hw,
    ...
    skb = lcompat_ieee80211_alloc_helper_skb(
        hw, sizeof(struct ieee80211_hdr_3addr) + (qos ? sizeof(__le16) : 0), 0);
    if (!skb)
        return NULL;
    hdr = skb_put_zero(skb, sizeof(struct ieee80211_hdr_3addr));
    if (!hdr) {
    ...
    lcompat_ieee80211_fill_3addr(hdr, fc, lcompat_ieee80211_vif_bssid(vif),
                                 vif ? vif->addr : NULL,
                                 lcompat_ieee80211_vif_bssid(vif));
    if (qos && !skb_put_zero(skb, sizeof(__le16))) {
        dev_kfree_skb_any(skb);
        return NULL;
    }
    IEEE80211_SKB_CB(skb)->control.vif = vif;
...
static inline struct sk_buff *ieee80211_tx_dequeue(struct ieee80211_hw *hw,
    ...
    if (!txq)
        return NULL;
    return skb_dequeue(&txq->lcompat_queue);
}
...
static inline void ieee80211_txq_get_depth(struct ieee80211_txq *txq,
    ...
    if (frame_cnt)
        *frame_cnt = txq ? skb_queue_len(&txq->lcompat_queue) : 0;
    if (byte_cnt)
        *byte_cnt = 0;
}
...
static inline u8 *ieee80211_get_qos_ctl(struct ieee80211_hdr *hdr) {
    return hdr ? ((u8 *)hdr) + ieee80211_hdrlen(hdr->frame_control) -
                     sizeof(__le16)
               : NULL;
}
